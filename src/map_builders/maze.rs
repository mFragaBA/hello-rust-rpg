...
impl MazeGrid {
    ...
    fn available_neighbors(&self, pos: Position) -> Vec<Position> {
        ...
        for pos in neighbors {
            ...
            if pos_idx != -1 && !self.is_visited[pos_idx as usize] { available_neighs.push(pos); }
        }
    ...
    fn generate(mut self, builder: &mut MazeBuilder, rng: &mut RandomNumberGenerator) -> Self {
        ...
        while let Some(current_pos) = stack.last().copied() {
            ...
            if let Some(neigh) = self.next_neighbor(current_pos, rng) {
                ...
                let map_idx = builder.map.xy_idx(current_pos.x * 2 + 1, current_pos.y * 2 + 1);

                builder.map.tiles[map_idx] = TileType::Floor;
                // account for tore down walls
                if self.walls[idx as usize] & NORTH != 0 { builder.map.tiles[map_idx - builder.map.width as usize] = TileType::Floor; }
                if self.walls[idx as usize] & SOUTH != 0 { builder.map.tiles[map_idx + builder.map.width as usize] = TileType::Floor; }
                if self.walls[idx as usize] & WEST != 0 { builder.map.tiles[map_idx - 1] = TileType::Floor; }
                if self.walls[idx as usize] & EAST != 0 { builder.map.tiles[map_idx + 1] = TileType::Floor; }
